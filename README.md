...
- 内置 800+ 条 Minecraft 社区标准术语表，AI 翻译自动对齐社区公认译名
...
项目根目录的 `translation_glossary.md` 是翻译术语库，翻译时会自动解析并注入提示词，确保 AI 使用社区公认的 Minecraft 中文译名。

- 按模组分类的 Markdown 表格，随时可编辑增删
- 支持常规术语表（`English | 中文`）和翻译陷阱表（`英文 | 错误译法 | 正确译法`）
- 修改后无需重启，下次运行翻译即刻生效
- 与内置原版术语自动合并，Markdown 文件优先级更高
